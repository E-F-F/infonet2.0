<x-hrms::layouts.master>
    <div class="max-w-5xl mx-auto px-4 py-6">
        <div class="flex justify-between items-center mb-4">
            <h2 class="text-2xl font-bold text-gray-800">Training Types</h2>
        ...
        <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
            <table class="w-full text-sm text-left text-gray-700" id="trainingTypesTable">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            const trainingTypesTable = document.getElementById('trainingTypesTable');
            ...
            trainingTypesTable.addEventListener('click', async (e) => {
                ...
                if (e.target.classList.contains('save-btn')) {
                    ...
                    let url, method;
                    if (isNew) {
                        url = '{{ route('hrms.training-types.store') }}';
                        method = 'POST';
                    } else {
                        url = `{{ url('hrms/training-types') }}/${rowId}`;
                    ...
                    try {
                        ...
                        if (data.success) {
                            row.dataset.id = data.trainingType.id;
                            row.querySelector('[data-field="name"]').innerText = data.trainingType.name;
                            makeRowViewable(row, data.trainingType.name);
                            alert(data.message);
                        } else {
                            alert('Failed to save training type: ' + data.message);
                        }
                    } catch (error) {
                        console.error(error);
                ...
                if (e.target.classList.contains('delete-btn')) {
                    ...
                    try {
                        const response = await fetch(`{{ url('hrms/training-types') }}/${rowId}`, {
                            method: 'DELETE',
                            headers: {
                        ...
                        if (data.success) {
                            row.remove();
                            alert(data.message || 'Training type deleted successfully.');
                        } else {
            ...
            addNewRowBtn.addEventListener('click', () => {
                const newRow = trainingTypesTable.querySelector('tbody').insertRow();
                newRow.classList.add('bg-white', 'border-b', 'hover:bg-gray-50', 'editing');

                newRow.innerHTML = `
            <td class="px-6 py-4 editable-cell" data-field="name">
                <input type="text" class="w-full px-2 py-1 border rounded" placeholder="New Training Type Name" />
            </td>
            <td class="px-6 py-4 flex gap-2 action-buttons">
