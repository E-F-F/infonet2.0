<x-hrms::layouts.master>
    <div class="max-w-5xl mx-auto px-4 py-6">
        ...
        <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
            <table class="w-full text-sm text-left text-gray-700" id="leaveTypesTable">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            leaveTypesTable.addEventListener('click', async (e) => {
                ...
                if (e.target.classList.contains('save-btn')) {
                    ...
                    if (newName === '') {
                        alert('Leave type name cannot be empty.');
                        return;
                    }
                    ...
                    let url, method;
                    if (isNew) {
                        url = '{{ route('hrms.leave-types.store') }}';
                        method = 'POST';
                    } else {
                        url = `{{ url('hrms/leave-types') }}/${rowId}`;
                    ...
                    try {
                        ...
                        if (data.success) {
                            row.dataset.id = data.leaveType.id;
                            row.querySelector('[data-field="name"]').innerText = data.leaveType.name;
                            makeRowViewable(row, data.leaveType.name);
                            alert(data.message);
                        } else {
                            alert('Failed to save leave type: ' + data.message);
                        }
                    } catch (error) {
                        console.error(error);
                ...
                if (e.target.classList.contains('delete-btn')) {
                    ...
                    try {
                        const response = await fetch(`{{ url('hrms/leave-types') }}/${rowId}`, {
                            method: 'DELETE',
                            headers: {
                        ...
                        if (data.success) {
                            row.remove();
                            alert(data.message || 'Leave type deleted successfully.');
                        } else {
            ...
            addNewRowBtn.addEventListener('click', () => {
                const newRow = leaveTypesTable.querySelector('tbody').insertRow();
                newRow.classList.add('bg-white', 'border-b', 'hover:bg-gray-50', 'editing');

                newRow.innerHTML = `
            <td class="px-6 py-4 editable-cell" data-field="name">
                <input type="text" class="w-full px-2 py-1 border rounded" placeholder="New Leave Type Name" />
            </td>
            <td class="px-6 py-4 flex gap-2 action-buttons">
