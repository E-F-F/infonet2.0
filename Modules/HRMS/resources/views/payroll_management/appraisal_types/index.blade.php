<x-hrms::layouts.master>
    <div class="max-w-5xl mx-auto px-4 py-6">
        <div class="flex justify-between items-center mb-4">
            <h2 class="text-2xl font-bold text-gray-800">Appraisal Types</h2>
        ...
        <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
            <table class="w-full text-sm text-left text-gray-700" id="appraisalTypesTable">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            const appraisalTypesTable = document.getElementById('appraisalTypesTable');
           ...
           appraisalTypesTable.addEventListener('click', async (e) => {
                ...
                if (e.target.classList.contains('save-btn')) {
                    ...
                    if (newName === '') {
                        alert('Appraisal type name cannot be empty.');
                        return;
                    }
                    ...
                    let url, method;
                    if (isNew) {
                        url = '{{ route('hrms.appraisal-types.store') }}';
                        method = 'POST';
                    } else {
                        url = `{{ url('hrms/appraisal-types') }}/${rowId}`;
                    ...
                    try {
                        ...
                        if (data.success) {
                            row.dataset.id = data.appraisalType.id;
                            row.querySelector('[data-field="name"]').innerText = data.appraisalType.name;
                            makeRowViewable(row, data.appraisalType.name);
                            alert(data.message);
                        } else {
                            alert('Failed to save appraisal type: ' + data.message);
                        }
                    } catch (error) {
                        console.error(error);
                ...
                if (e.target.classList.contains('delete-btn')) {
                    if (!confirm('Are you sure you want to delete this appraisal type?')) return;

                    try {
                        const response = await fetch(`{{ url('hrms/appraisal-types') }}/${rowId}`, {
                            method: 'DELETE',
                            headers: {
                        ...
                        if (data.success) {
                            row.remove();
                            alert(data.message || 'Appraisal type deleted successfully.');
                        } else {
                            alert('Failed to delete appraisal type.');
                        }
                    } catch (error) {
                        console.error(error);
            ...
            addNewRowBtn.addEventListener('click', () => {
                const newRow = appraisalTypesTable.querySelector('tbody').insertRow();
                newRow.classList.add('bg-white', 'border-b', 'hover:bg-gray-50', 'editing');

                newRow.innerHTML = `
            <td class="px-6 py-4 editable-cell" data-field="name">
                <input type="text" class="w-full px-2 py-1 border rounded" placeholder="New Appraisal Type Name" />
            </td>
            <td class="px-6 py-4 flex gap-2 action-buttons">
